...
function send_password_mail($email, $pwd)
{
  ...
  try {
    ...
    $mail->Body    = "
                     \n Your Username and Password for VaX-FarE Portal login is Provided below.\n\n\n
                     Username : $email \n
                     Password : $pwd \n\n\n
...
if (isset($_SESSION['uid'])) {
  ...
  if (isset($_POST["sub"])) {
    ...
    if ($re1) {
      ...
      $query = "insert into tbl_vaccinator(lid,emp,fname,lname,mob,house,dist,city,centre,exp,status,role) values('$last_id','$uname','$fname','$lname','$mob','$house','$dist','$city','$centre','$exp','1','$role')";
      $re = mysqli_query($con, $query);

      $check_email = "SELECT username from tbl_login where username='$mail' and password='$pwd';";
          ...
          <div class="row">
            ...
            <div class="col-75">
              <select name="role" id="role" required>
                ...
                <?php
                $qy = "select * from tbl_role";
                $rt = mysqli_query($con, $qy);
                while ($r3 = mysqli_fetch_array($rt)) {
                ?>
                  <option value=" <?php echo $r3['role_id'];?> ">
                    <?php echo $r3['role']; ?>
                  </option>
                <?php
                }
