...
<?php 
 ...
 require("connect.php");
     ...
     {
      $lid = $_SESSION["uid"];
      $query2="select * from tbl_vctr_stock where status=1;";
      $re2=mysqli_query($con,$query2);
  $rww=mysqli_num_rows($re2);
  $q="select * from tbl_vctr_stock where status=0;";
...
<table style="border: thick solid red;">
        <thead>
        <tr>
                <th colspan="6"><strong>Stock Request</strong>
                </th>
            </tr>
        </thead>
        <tbody>
      ...
      $query3="select * from tbl_centre where centre_id='$cid';";
        ...
        ?>
            <tr>
              ...
              <td> <?php echo $row3['name'] ;?></td>  
                ...
                <td>
              <a href='approve.php?new_id=<?php echo $row['stock_id'];?>'><button type="submit" name="approve" class="delete-btn" >Approve</button></a>
 ...
 <?php   
  ...
  ?>  
   <table>
        <thead>
        <tr>
                <th colspan="6"><b>Total Stock Request Details</b>
                </th>
            </tr>
        </thead>
        <tbody>
      ...
      while ($rw=mysqli_fetch_array($rq))
      {
        $rid=$rw['vaccine'];
        $rcid=$rw['centre'];
      $queryr1="select * from tbl_vaccine where vaccine_id='$rid';";
      $rer1=mysqli_query($con,$queryr1);
      $rw2=mysqli_fetch_array($rer1);
      $queryr3="select * from tbl_centre where centre_id='$rcid';";
      $rer3=mysqli_query($con,$queryr3);
      $rw3=mysqli_fetch_array($rer3);
        ?>
            <tr>
              <td><?php echo  $i++; ?></td>
              <td> <?php echo $rw3['name'] ;?></td>  
                <td> <?php echo $rw2['vaccine'] ;?></td>
                <td> <?php echo $rw['amt'] ;?></td>
                <td> <?php echo $rw['date_add'] ;?></td>  
