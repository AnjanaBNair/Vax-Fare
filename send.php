<?php 
  ...
  function send_password_mail($email, $token,$pwd)
  {
    ...
    try {
      ...
      $mail->setFrom('anjananairb2312@gmail.com', 'anjana');
      ...
      $mail->Body    = "VaX-FarE 
                        You are added as a vaccinator in the covid-vaccination portal vax-fare.
...
if(isset($_POST['check_mail']))
{
    ...
    if(mysqli_num_rows($check_email_run)>0)
    {
     ...
     if(isset($_POST["sub"]))
     {
      ...
      $exp=$_POST["exp"];
      $pwd=$_POST["pwd"];
      $query4="select * from tbl_login where username='$mail'";
      $re4=mysqli_query($con,$query4);
      $count=mysqli_num_rows($re4);
		if($count>0)
		{
      $query5="update tbl_login set status=2 where username='$mail'";
      $re5=mysqli_query($con,$query5);
      $qq="select * from tbl_login where status=2 and username='$mail'";
      $req=mysqli_query($con,$qq);
      $rw=mysqli_fetch_array($req);
      $new_id=$rw['id'];
      $query6="update tbl_vaccinator set status=1 where lid='$new_id'";
      $re6=mysqli_query($con,$query6);
      $token=md5(rand());
      $check_email="SELECT username from tbl_login where username='$mail'";
      $check_email_run=mysqli_query($con, $check_email);
      if(mysqli_num_rows($check_email_run)>0)
      {
          $row=mysqli_fetch_assoc($check_email_run);
          $update_token="UPDATE tbl_login SET verify_token='$token' where username='$mail'";
          $update_token_run=mysqli_query($con,$update_token);
          if($update_token_run)
          {
              send_password_mail($email, $token);
      ...
      $query1="insert into tbl_login(username,password,status) values('$mail','$pwd','2')";
      $re1=mysqli_query($con,$query1);
      if($re1)
      {
        $last_id=mysqli_insert_id($con);
        $query="insert into tbl_vaccinator(lid,emp,fname,lname,mob,house,dist,city,centre,exp,status) values('$last_id','$uname','$fname','$lname','$mob','$house','$dist','$city','$centre','$exp','1')";
      ...
      if(mysqli_num_rows($check_email_run)>0)
      {
          ...
          if($update_token_run)
          {
              send_password_mail($email, $token,$pwd);
          }
          else{
              echo "<script> alert('Wrong'); </script>";
          }
...
<body>
  ...
  <section class="home-section">
          ...
          <div class="row">
            ...
            <div class="col-75">
              <input type="text" id="emp" name="emp"
                value="<?php echo  $unique_id = 'emp-' . sprintf('%04d', mt_rand(1, 999)); ?>" />
            </div>
          </div>
          <div class="row">
            <div class="col-25">
    ...
    ?>
          ...
          <div class="row">
            <div class="col-25">
              <label for="centre">Centre Assigned</label>
            </div>
            <div class="col-75">
              <select name="centre" id="centre" required>
                <option value="" disabled selected>---- Choose centre ----</option>
                <?php 
      $query3="select * from tbl_centre";
      $result3=mysqli_query($con,$query3);
      while ($row3 = mysqli_fetch_array($result3))
      {
      ?>
                <option value=" <?php echo $row3['centre_id']; ?> ">
                  <?php echo $row3['name']; ?>
                </option>
                <?php
    }
